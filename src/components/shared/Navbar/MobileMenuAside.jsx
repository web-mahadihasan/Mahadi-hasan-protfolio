...
const MobileMenuAside = ({ setOpenMenu, openMenu }) => {
  ...
  return (
    <div
      className={`absolute lg:hidden z-40 duration-700 min-h-[calc(100vh-50px)] ${
        openMenu ? "top-0 left-0 block" : "-left-[800px] top-0"
      }`}
    >
      <div className="menu flex flex-col justify-between bg-base-200 text-base-content min-h-screen w-80 p-4 overflow-y-scroll">
        ...
        <div className="flex items-center justify-between mt-5">
          <div className="flex items-center gap-2">
            <Link
              to={"/"}
              className=" text-4xl font-bold font-rubik tracking-wider text-main"
            >
              Mahadi
            </Link>
          </div>
          ...
          <button
            ...
            className="p-2 border border-black/65 cursor-pointer rounded-full"
          >
            ...
            {navLinks.map((link) => (
              <li key={link.element}>
                <a
                  href={link.path}
                  className={
                    "font-normal text-[16px] text-dark-black font-rubik uppercase flex justify-between"
                  }
                >
                  {link.element} <RiArrowRightSLine size={22} />
                </a>
              </li>
            ))}
          </ul>
        ...
        <div className="py-1">
          ...
          <div className="flex items-center gap-3">
            <div className="px-2 border-r-2 border-gray-500">
              <LuPhoneCall size={28} className="text-main" />
            </div>
            <div>
              <p className="font-medium text-base">More Inquery</p>
              <h4 className="text-main font-semibold text-lg">
                +8801794943980
              </h4>
            </div>
          </div>
